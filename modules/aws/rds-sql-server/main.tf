...
resource "aws_db_instance" "this" {
  ...
  final_snapshot_identifier   = var.skip_final_snapshot ? null : "${local.name}-sqlserver-final"
  tags                        = var.tags
}
...
resource "aws_security_group" "nlb" {
  count       = local.enable_private_link ? 1 : 0
  name        = "${local.name}-nlb"
  description = "PrivateLink NLB for SQL Server"
  vpc_id      = module.network.vpc_id
  tags        = var.tags
}
...
resource "aws_lb_target_group" "this" {
  count              = local.enable_private_link ? 1 : 0
  name               = substr("${local.name}-sql", 0, 32)
  port               = local.port
  protocol           = "TCP"
  target_type        = "ip"
  vpc_id             = module.network.vpc_id
